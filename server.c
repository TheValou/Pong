...
** Last update Sat Aug  5 00:49:55 2017 WURFL Valentin
*/
...
void	connect_client(t_server *server, t_data *data)
{
 ...
 if (cs > 0)
 {
   while (42)
   {
    ...
    printf("le jeu continue !! La valeur de read = %d\n", rread);
    ...
    if(data->ball_y <=0)
     data->yBall = BALL_SPEED; /* va vers le bas */
      ...
      /* Mise a jour du score */
        ...
        sprintf(P1_x_s,"%d",data->xplayer);
        sprintf(P1_y_s,"%d",data->yplayer);
        sprintf(P2_x_s,"%d",data->xplayertwo);
        sprintf(P2_y_s,"%d",data->yplayertwo);
